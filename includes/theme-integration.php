...
function wns_handle_theme_download_subscription() {
    ...
    if (!wns_is_allowed_email_domain($email)) {
        return; // Skip non-allowed domains
    }
...
function wns_theme_integration_notice() {
    ...
    if (has_action('foxiz_subscribe', 'wns_handle_theme_download_subscription')) {
        ...
        echo esc_html__('Download form submissions are automatically being added to your newsletter subscriber list (Gmail, Hotmail, Yahoo only).', 'wp-newsletter-subscription');
        echo '</p>';
        echo '</div>';
    }
}
            ...
            <td>
                <p class="description">
                    <?php if (has_action('foxiz_subscribe', 'wns_handle_theme_download_subscription')): ?>
                        <span style="color: green;">✓ <?php _e('Active - Download form emails are automatically added to newsletter subscribers (Gmail, Hotmail, Yahoo only)', 'wp-newsletter-subscription'); ?></span>
                    <?php else: ?>
...
// CRITICAL: Override the theme's download form handler to validate email domains
...
function wns_override_foxiz_form_download_submit() {
    ...
    // NEW: Validate email domain (only allow Gmail, Hotmail, Yahoo)
    if (!wns_is_allowed_email_domain($email)) {
        wp_send_json([
            'success' => false,
    ...
    // If validation passes, continue with the original theme logic
    // Remove our hook to prevent infinite loop
    remove_action('wp_ajax_form_download_submit', 'wns_override_foxiz_form_download_submit', 5);
    remove_action('wp_ajax_nopriv_form_download_submit', 'wns_override_foxiz_form_download_submit', 5);
    
    // Call the original theme function if it exists
    if (function_exists('foxiz_form_download_submit')) {
        foxiz_form_download_submit();
    } else {
        // Fallback if theme function doesn't exist
        wp_send_json([
            'success' => false,
            'message' => esc_html__('Download function not available.', 'wp-newsletter-subscription'),
        ]);
    }
}
